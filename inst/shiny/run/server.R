server <- function(input, output) {
  ...
  # Scatterplot output
  output$scatterplot <- renderPlot({ggplot2::
  ...
  output$distribution1 <- renderPlot({
    ggplot2::ggplot(data=Ecommerce_Customersxx, aes(Ecommerce_Customersxx$`Avg. Session Length`)) +
  ...
  output$distribution2 <- renderPlot({
    ggplot2::ggplot(data=Ecommerce_Customersxx, aes(Ecommerce_Customersxx$`Yearly Amount Spent`)) +
